...
const Appointments = () => {
  ...
  const fetchAppointments = async () => {
    ...
    const response = await fetch("https://telemedicine-0i2m.onrender.com/api/appointments", {
              ...
              {role === "doctor" && appt.status === "Pending" && (
                <Box mt={2} display="flex" gap={2} justifyContent="flex-end">
                  <Button
                    ...
                    onClick={async () => {
                      const token = localStorage.getItem("token");
                      await fetch(`https://telemedicine-0i2m.onrender.com/api/appointments/update/${appt._id}`, {
                        method: "PATCH",
                        headers: {
